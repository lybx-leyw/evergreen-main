/// 对话消息数据模型 — 对应 reasonix/internal/provider/provider.go。
library;

/// 对应 Go 的 provider.Role。
enum Role {
  system,
  user,
  assistant,
...
/// 模型发出的工具调用请求。对应 Go 的 provider.ToolCall。
class ToolCall {
  final String id;
  final String name;
...
/// 暴露给模型的工具定义。对应 Go 的 provider.ToolSchema。
class ToolSchema {
  final String name;
...
/// 单条对话消息。对应 Go 的 provider.Message。
class Message {
  final Role role;
...
/// 移除孤立的 tool 消息，修复 tool_calls 配对。对应 Go 的 provider.SanitizeToolPairing()。
