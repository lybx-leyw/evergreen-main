/// Python 环境管理——统一解释器路径发现（单例收敛）、依赖检查、OCR 子进程执行。
///
/// ⚠️ 副本 — 与 `lib/core/utils/python_env.dart` 同步（子包测试隔离用）。
///
/// # 统一解释器路径（唯一事实源）
///
...
class PythonEnv {
  ...
  Future<String?> get pythonExe async => PythonInterpreter.instance
      .resolveExePath(configuredPath: _configuredPython);
