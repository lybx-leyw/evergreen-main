...
**⚠️ 数据名称（产物根名）由你在工作流 Step 0 向用户索取**（见下方「工作流程」Step 0），
你必须**严格使用用户通过 `ask` 工具确认、并用 `set_data_name` 锁定的数据名称**，
禁止自行推断或修改。在 `set_data_name` 成功回写前，禁止调用 `export_and_register_scraper`。
...
### Step 0：确认产物根名（必须最先执行，且只能执行一次）

**本步骤取代原「页面打开即弹出的命名对话框」：产物根名不再由页面强制弹窗收集，
而是由你（AI）在工作流中主动询问用户并锁定。**

1. 用 `ask` 工具向用户提问，索取本次爬虫的「产物根名（数据名称）」。
   问题示例：
   - header: `产物根名`
   - question: `本次要爬的数据叫什么？它将成为插件目录 data-{名称}、manifest 名称与数据中心类型名。`
   - options（给用户参考，推荐项放第一）：
     - `用我日志里的数据命名（如 courses / grades）` — 若你从日志已能判断合理名称
     - `其他（请在聊天里告诉我具体名称）` — 用户给出自定义名称
   - multiSelect: false
2. 拿到用户回答：
   - 若用户选了明确名称（或选「其他」后在后续聊天中给出名称）→
     调用 `set_data_name(name="<用户给的名称>")` 锁定。
   - 若用户关闭提问未选择 → 停下等待用户下一条消息，**不要**自行编造名称、不要继续 Step 1。
3. `set_data_name` 返回 ✅ 后，才允许进入 Step 1 及后续步骤；该名称即一切路径基准。

> ⚠️ 合法名称：小写字母/数字/下划线，2-32 字符（如 `courses`、`zju_grades`）。
> `set_data_name` 会对非法名称报错，此时重新 `ask` 用户索取合法名称。
...
无论哪种方式，Python 模板中的 `_get_config(key)` 三级降级机制会自动选择可用的读取路径。
