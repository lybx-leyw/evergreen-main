...
import 'package:evergreen_base/renderer/components/shared/widgets/slide_canvas.dart';
import 'package:evergreen_base/renderer/components/shared/widgets/slide_sorter.dart';
import 'package:evergreen_base/renderer/components/shared/widgets/speaker_notes_panel.dart';
...
class _PresentationViewState extends State<PresentationView> {
  ...
  /// 从 manifest config 读取幻灯片列表；无数据时给出默认占位。
  List<SlideData> get _slides {
    final raw = widget.component?.config['slides'];
    if (raw is List) {
      return raw.whereType<Map<String, dynamic>>().map((e) {
        return SlideData(
          title: e['title'] as String? ?? '',
          layout: e['layout'] as String? ?? 'content',
          content: e['body'] as String? ?? e['content'] as String?,
        );
      }).toList();
    }
    return const <SlideData>[
      SlideData(title: '未配置幻灯片', layout: 'title'),
    ];
  }
