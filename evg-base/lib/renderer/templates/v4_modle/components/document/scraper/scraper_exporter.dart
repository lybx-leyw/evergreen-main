...
def _get_config(key):
    ...
    设计原因：ConfigHttpServer 可能存在同步延迟（新注入的配置项），
    此时降级到环境变量确保脚本仍可运行。
    """
    ...
    try:
        ...
        if port_file:
            ...
            ConnectionError, TimeoutError, OSError, json.JSONDecodeError, Exception):
        pass  # 静默降级到环境变量
    ...
    raise RuntimeError(
        f'无法获取配置 "{key}"：\n'
        f'  1. ConfigHttpServer 不可用（检查 .config_port 是否存在于项目根目录）\n'
        f'  2. 环境变量未设置\n'
        f'  → 修复方法：在终端执行 set {key}=<值>  （Windows / PowerShell）\n'
        f'  → 或在 Evergreen 设置面板中注册此配置项'
    )
...
Future<ExportResult> exportAsPython(
  ...
  try {
    ...
    final readerContent = scraperConfigTemplate.replaceFirst(
      '{CREDENTIAL_PLACEHOLDER}',
