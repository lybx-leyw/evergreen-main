...
///
/// 自参考工程 `cp_evergreen_push/lib/features/auth/services/zjuam_service.dart`
/// 改造：错误/日志/Result 复用 evg-base 的 `core/errors.dart` / `core/log.dart` /
/// `core/result.dart`（API 与参考工程一致）。
///
...
class ZjuAmService {
  ...
  Future<Result<Cookie>> login(String username, String password) async {
    ...
    try {
      ...
      // Step 3: RSA encrypt
      final pwdEnc = rsaEncrypt(password, modulus, exponent);
  ...
  ///
  /// 公开静态方法，便于离线单测（不依赖网络与登录态）。
  static String rsaEncrypt(String plaintext, String modulusHex, String exponentHex) {
