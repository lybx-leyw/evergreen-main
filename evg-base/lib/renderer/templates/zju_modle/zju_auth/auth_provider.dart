...
/// 认证状态——ZJU SSO 登录态（iPlanetDirectoryPro + synjones-auth）。
///
/// 自参考工程 `cp_evergreen_push/lib/features/auth/providers/auth_provider.dart`
/// 改造：凭证源从 `AppConfig.zjuUsername/zjuPassword` 改为
/// `getSetting(prefs, 'ZJU_USERNAME'/'ZJU_PASSWORD')`（evg-base 设置中枢），
/// provider 名加 `zju` 前缀避免与全局 providers 冲突。
class AuthState {
  ...
  /// BlueWare / elife platform bearer token for ecard queries.
  /// Obtained separately from SSO; may be null if BlueWare login hasn't
  /// been automated yet.
  final String? synjonesAuthToken;
  ...
  AuthState copyWith({
    ...
    return AuthState(
      ...
      error: error,
      synjonesAuthToken: synjonesAuthToken ?? this.synjonesAuthToken,
    );
  }
...
class AuthNotifier extends StateNotifier<AuthState> {
  ...
  /// Perform RSA login with credentials from settings (ZJU_USERNAME/ZJU_PASSWORD).
  Future<bool> login() async {
    final username = getSetting(_prefs, 'ZJU_USERNAME');
    final password = getSetting(_prefs, 'ZJU_PASSWORD');
  ...
  Future<bool> restoreSession() async {
    ...
    if (ssoCookie != null && ssoCookie.isNotEmpty) {
      ...
      // Restore BlueWare token if persisted
      final synjonesToken = cookieStore.synjonesAuthToken;

      AuthInterceptor.onRelogin = () async => login();
      state = AuthState(
        isLoggedIn: true,
        ssoCookie: cookie,
        synjonesAuthToken: synjonesToken,
      );
      return true;
    }
  ...
  Future<void> logout() async {
    ...
    await cookieStore.clearSynjonesAuthToken();
...
final zjuHttpClientProvider = Provider<HttpClient>((ref) {
  return HttpClient()
    ..userAgent =
        'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36';
});

final zjuAuthProvider =
    StateNotifierProvider<AuthNotifier, AuthState>((ref) {
  final dio = ref.read(zjuDioClientProvider);
  final cookieJar = ref.read(zjuCookieJarProvider);
  final httpClient = ref.read(zjuHttpClientProvider);
  final prefs = ref.read(sharedPreferencesProvider);
  return AuthNotifier(dio, cookieJar, httpClient, prefs);
});
