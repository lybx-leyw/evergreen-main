...
import 'package:evergreen_base/renderer/widgets/type_check_input.dart';
import 'package:evergreen_base/renderer/widgets/flashcard_view.dart';
...
class _TypeCheckSlotState extends State<TypeCheckSlot> {
  ...
  void _loadConfig() {
    ...
    _loadWords(cfg['wordList'] as String? ?? 'words.json');
  }

  void _loadWords(String wordListFile) {
    try {
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          // 闪卡（固定高度，避免 Expanded 在 SingleChildScrollView 无界约束中报错）
          ConstrainedBox(
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          // 答题区域（固定高度，避免 Expanded 在 SingleChildScrollView 无界约束中报错）
          ConstrainedBox(
            constraints: const BoxConstraints(minHeight: 120, maxHeight: 300),
            child: _buildAnswerArea(theme, word),
          ),
