...
/// 用法：
/// ```dart
/// final css = LayoutEngine.buildContainerCss({'type': 'grid', 'preset': {'columns': 2, 'gap': 8}});
...
List<String> _buildDockCss(Map<String, dynamic> p) {
  ...
  final topH = ((p['top'] as Map<String, dynamic>?)?['height'] as num?)?.toInt() ?? 0;
  final bottomH = ((p['bottom'] as Map<String, dynamic>?)?['height'] as num?)?.toInt() ?? 0;
  final leftW = ((p['left'] as Map<String, dynamic>?)?['width'] as num?)?.toInt() ?? 0;
  final rightW = ((p['right'] as Map<String, dynamic>?)?['width'] as num?)?.toInt() ?? 0;
