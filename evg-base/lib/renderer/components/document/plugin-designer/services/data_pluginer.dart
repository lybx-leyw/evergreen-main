...
class DataPluginer {
  ...
  Future<DataPluginResult> registerDataPlugin({
    ...
    try {
      ...
      // 4) 确保 module/ 根目录存在一个占位 module manifest（后续 P2 编排会更新）
  ...
  Map<String, dynamic> _buildDataManifest({
    ...
    return {
      ...
      'description': '爬虫自动生成的数据源: ${schema.sourceUrl}',
      'endpoint': endpoint,
      'refreshInterval': refreshInterval,
      'fields': schema.fields.map((f) => f.toJson()).toList(),
      'source': {
        'url': schema.sourceUrl,
        'method': 'auto',
      },
      'process': [
        {
          'exe': 'data/scraper.exe',
          'protocol': 'http',
        }
      ],
    };
