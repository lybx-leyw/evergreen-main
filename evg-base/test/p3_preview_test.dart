...
import 'package:evergreen_base/renderer/components/document/plugin-designer/services/auto_compile_service.dart';
import 'package:evergreen_base/renderer/components/document/plugin-designer/services/design_to_manifest.dart';
import 'package:evergreen_base/renderer/components/document/plugin-designer/view/preview_panel.dart';
...
void main() {
  ...
  group('P3 PreviewPanel', () {
    ...
    testWidgets('单页面单 Slot 正确渲染预览', (tester) async {
    ...
    testWidgets('多 Slot 按 region 排序展示', (tester) async {
      final doc = DesignDocument(
        pluginId: 'multi-slot',
        pages: [
          DesignPage(
      ...
      expect(find.text('中心'), findsOneWidget);
    ...
    testWidgets('多页面支持页面切换', (tester) async {
      final doc = DesignDocument(
        pluginId: 'multi-page',
        pages: [
    ...
    testWidgets('空 Slot 显示未绑定提示', (tester) async {
      final doc = DesignDocument(
        pluginId: 'empty-slot',
        pages: [
      ...
      expect(find.textContaining('未绑定组件'), findsOneWidget);
    });
  });
