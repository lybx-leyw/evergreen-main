...
- **v1.3.1（当前版本）** — 在 v1.3.0 基础上的修复迭代；UI 溢出修复 + Palace 过滤栏布局优化，功能模块 17 个。
...
### zju-connect — RVPN 代理客户端

- **上游项目**: zju-connect — 浙江大学 RVPN SOCKS5 代理客户端
- **作者**: [@Mythologyli](https://github.com/Mythologyli)
- **仓库**: https://github.com/Mythologyli/zju-connect
- **许可证**: GNU General Public License v3.0 (GPL-3.0)

Evergreen 的 **RVPN** 模块将 zju-connect 作为外部子进程调用，通过启动 `zju-connect -server rvpn.zju.edu.cn -port 443 -username <学号> -password <密码>` 在本地建立 SOCKS5 代理（`127.0.0.1:1080`），实现校园网资源的校外访问。

**引用范围：**

| 模块 | 说明 |
|---|---|
| `features/rvpn/` | Dart/Flutter UI 调用 zju-connect 二进制，管理进程生命周期、健康检查、日志捕获 |
| `vendor/zju-connect/` | 用户在构建时自行下载的 zju-connect 可执行文件（已加入 `.gitignore`，不纳入版本控制） |

zju-connect 二进制不在本仓库的版本控制中。首次使用 RVPN 模块时，请参考 `BUILD.md` 中的下载安装说明。
...
### Evergreen Agents — 多 Agent 联邦协作框架

- **子项目**: `agent_contributing/evergreen_agents/`
- **许可证**: GPL-3.0-only

Evergreen 的百级多 Agent 联邦协作框架——一个支持 ~100 个 AI Agent 并行工作的 CLI 系统，每个 Agent 独立且长期负责一个代码模块。架构设计受 DeepSeek-Reasonix（事件驱动/工具注册表/权限门禁）和 TradingAgents（角色工厂/双层级 LLM/StateGraph 编排）启发，以 Python 从零编写。

详细的架构参考致谢见 [agent_contributing/evergreen_agents/ATTRIBUTION.md](agent_contributing/evergreen_agents/ATTRIBUTION.md)。
...
**Evergreen Multi-Tools v1.3.1** 以 **GNU General Public License v3.0 (GPL-3.0)** 发布。
