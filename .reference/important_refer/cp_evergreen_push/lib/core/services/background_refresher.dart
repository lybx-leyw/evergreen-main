...
import '../../features/autosign/providers/autosign_provider.dart';
...
class BackgroundRefresher {
  ...
  Future<void> _silentRefresh() async {
    try {
      ...
      // Autosign background check — if the user has enabled background autosign,
      // each refresh tick will also check for pending rollcalls.
      try {
        final autosignBgEnabled = _ref.read(autosignBackgroundEnabledProvider);
        if (autosignBgEnabled) {
          final autosignSvc = _ref.read(autosignServiceProvider);
          await autosignSvc.checkRollcallsOnce();
        }
      } catch (_) {}
