/// OpenVPN configuration generator for ProtonVPN.
///
/// Produces a complete `.ovpn` config file at runtime for a given
/// server endpoint. Embedded keys/certs are ProtonVPN public infrastructure
/// extracted from the reference:
///   .reference/win-app/src/ProtonVPN.Vpn/OpenVpn/ConfigTemplate.cs
///
/// Usage:
///   final config = ProtonVpnOvpnConfig.generate(serverIp: '1.2.3.4', port: 51820, ...);
///   await File(configPath).writeAsString(config);

class ProtonVpnOvpnConfig {
  /// Generate a complete OpenVPN client configuration string.
  ///
  /// The generated config uses:
  ///   - AES-256-GCM cipher
  ///   - TUN device
  ///   - ProtonVPN's embedded tls-crypt pre-shared key
  ///   - ProtonVPN's Root CA certificate
  ///   - auth-user-pass for credential passing (via management interface)
  static String generate({
    required String serverIp,
    required int port,
    required String protocol, // "udp" or "tcp"
  }) {
    return '''
client
dev tun
resolv-retry infinite
cipher AES-256-GCM
verb 3
tun-mtu 1500
tun-mtu-extra 32
mssfix 1450
persist-key
persist-tun
ping 15
ping-restart 0
ping-timer-rem
reneg-sec 0
hand-window 15
connect-timeout 10
connect-retry-max 1
remote-cert-tls server
register-dns
script-security 2
key-direction 1
disable-dco
remote $serverIp $port $protocol
$_kTlsCrypt
$_kRootCa
auth-user-pass
''';
  }

  // ═══════════════════════════════════════════════════════════
  // ProtonVPN tls-crypt pre-shared key
  // Source: ConfigTemplate.cs lines 76-96
  // This is part of ProtonVPN's public server infrastructure.
  // ═══════════════════════════════════════════════════════════

  static const String _kTlsCrypt = '''<tls-crypt>
-----BEGIN OpenVPN Static key V1-----
6acef03f62675b4b1bbd03e53b187727
423cea742242106cb2916a8a4c829756
3d22c7e5cef430b1103c6f66eb1fc5b3
75a672f158e2e2e936c3faa48b035a6d
e17beaac23b5f03b10b868d53d03521d
8ba115059da777a60cbfd7b2c9c57472
78a15b8f6e68a3ef7fd583ec9f398c8b
d4735dab40cbd1e3c62a822e97489186
c30a0b48c7c38ea32ceb056d3fa5a710
e10ccc7a0ddb363b08c3d2777a3395e1
0c0b6080f56309192ab5aacd4b45f55d
a61fc77af39bd81a19218a79762c3386
2df55785075f37d8c71dc8a42097ee43
344739a0dd48d03025b0450cf1fb5e8c
aeb893d9a96d1f15519bb3c4dcb40ee3
16672ea16c012664f8a9f11255518deb
-----END OpenVPN Static key V1-----
</tls-crypt>''';

  // ═══════════════════════════════════════════════════════════
  // ProtonVPN Root CA certificate (2026)
  // Source: ConfigTemplate.cs lines 98-136
  // ═══════════════════════════════════════════════════════════

  static const String _kRootCa = '''<ca>
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
</ca>''';
}
