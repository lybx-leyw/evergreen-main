...
class WordPeckerNotifier extends StateNotifier<WordPeckerState> {
  ...
  /// Grade the current word with an FSRS rating and advance to next.
  ...
  /// Advance to the next due word, or exit practice mode if none.
  void _advanceToNextWord() {
    final next = getNextDueWord();
    if (next != null) {
      state = state.copyWith(currentWord: next, etymologyResult: null);
      if (state.autoEtymology) {
        lookupEtymology(next);
      }
    } else {
      state = state.copyWith(isPracticing: false, currentWord: null, etymologyResult: null);
    }
  }
