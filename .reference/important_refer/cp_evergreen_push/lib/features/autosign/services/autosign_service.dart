...
class AutosignService {
  ...
  /// Public entry point for background refresh — checks rollcalls once.
  /// Returns immediately; errors are surfaced via the log stream.
  Future<void> checkRollcallsOnce() async {
    await _checkRollcalls();
  }
  ...
  Future<void> _checkRollcalls() async {
    try {
      ...
      for (final rc in rollcalls) {
        ...
        log('  📌 正在应答签到 #$rid: $title');
        final ok = await _tryRadarSignIn(rid as int);
        log(ok ? '  ✅ 签到成功 #$rid' : '  ❌ 签到失败 #$rid');
      }
    } catch (e) {
  ...
  /// Log a message to the stream (public, usable by the notifier).
