...
class ClassroomCrawler {
  ...
  /// Download all slide images to a local directory.
  ///
  /// Creates [destDir] if it doesn't exist. Saves each slide as
  /// `page_{N}.png` (or preserves the extension from the source URL).
  /// Returns the list of saved file paths on success.
  Future<Result<List<String>>> downloadSlides(
    List<PptSlide> slides,
    String destDir, {
    void Function(int completed, int total)? onProgress,
  }) async {
    _d('L3', 'downloadSlides() → ${slides.length} slides to $destDir');
    _sw.reset();
    _sw.start();
    ...
    try {
      ...
      Log().warn('ClassroomCrawler.downloadSlides: cannot create dir', error: e);
    ...
    for (int i = 0; i < total; i++) {
      ...
      // Extract extension from URL path (e.g. ".png", ".jpg"), default to ".png"
      ...
      try {
        ...
        _d('L1', 'downloadSlides: page ${slide.page} saved ($filePath)');
      } catch (e) {
        Log().warn('ClassroomCrawler.downloadSlides: page ${slide.page} failed',
            error: e);
    ...
    _d('L3',
        'downloadSlides() → ${savedPaths.length}/$total slides in ${_sw.elapsedMilliseconds}ms');

    if (savedPaths.isEmpty) {
      return Err(AppError.dataIntegrity(
          'ClassroomCrawler', 'savedPaths', 'non-empty list', 'empty list'));
    }
