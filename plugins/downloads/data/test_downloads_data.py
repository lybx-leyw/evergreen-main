"""downloads_data 自包含测试。
模式 A：无凭据时 CLI 报配置缺失错误（exit 1）。
模式 B：有 .env 凭据时真实拉取 courses 课程附件清单。
"""
import json
import os
import subprocess
...
def _run(args):
    return subprocess.run([sys.executable, os.path.join(_HERE, "downloads_data.py"), *args],
...
def test_cli_missing_config():
    ...
    try:
        r = _run(["--type", "downloads", "--project-root", _ROOT])
        assert r.returncode == 1
        assert "error" in json.loads(r.stdout)
    finally:
