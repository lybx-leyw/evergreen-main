...
真实调用 ZDBK 成绩单接口，断言返回 grades 非空且字段齐全。

运行: python test_scores_data.py
"""
import json
import os
import subprocess
...
def _run(args):
    return subprocess.run(
        [sys.executable, os.path.join(_HERE, "scores_data.py"), *args],
...
def test_cli_missing_config():
    ...
    try:
        r = _run(["--type", "scores_transcript", "--project-root", _ROOT])
...
def test_real_pull():
    ...
    obj = json.loads(r.stdout)
    assert "grades" in obj, f"响应缺少 grades: {r.stdout[:300]}"
    assert isinstance(obj["grades"], list), "grades 应为列表"
    assert len(obj["grades"]) > 0, "真实拉取 grades 不应为空"
    g0 = obj["grades"][0]
    for key in ("id", "name", "credit", "original", "fivePoint"):
        assert key in g0, f"grade 缺少字段 {key}: {g0}"
