"""training_plans_data 自包含测试。

模式 A（离线结构校验）：无凭据时，断言 CLI 报出配置缺失错误（exit 1 + error JSON）。
模式 B（真实拉取）：若项目根存在 .env，注入环境变量后真实调用 ZDBK 培养方案接口，
断言管道成功且返回结构合法（列表可能为空，属正常数据状态，非空时校验字段）。

运行: python test_training_plans_data.py
"""
import json
import os
import subprocess
...
def _run(args):
    return subprocess.run(
        [sys.executable, os.path.join(_HERE, "training_plans_data.py"), *args],
...
def test_cli_missing_config():
    ...
    try:
        r = _run(["--type", "training_plans", "--project-root", _ROOT])
...
def test_real_pull():
    ...
    obj = json.loads(r.stdout)
    assert "plans" in obj, f"响应缺少 plans: {r.stdout[:300]}"
    assert isinstance(obj["plans"], list), "plans 应为列表"
    assert "total" in obj, f"响应缺少 total: {r.stdout[:300]}"
    if obj["plans"]:
        p0 = obj["plans"][0]
        for key in ("planName", "planNo"):
            assert key in p0, f"plan 缺少字段 {key}: {p0}"
