"""zdbk_notifications_data 自包含测试。

模式 A（离线结构校验）：无凭据时，断言 CLI 报出配置缺失错误（exit 1 + error JSON）。
模式 B（真实拉取）：若项目根存在 .env，注入环境变量后真实调用 ZDBK 通知接口，
断言管道成功且返回结构合法（列表可能为空，属正常数据状态，非空时校验字段）。

运行: python test_zdbk_notifications_data.py
"""
import json
import os
import subprocess
...
def _run(args):
    return subprocess.run(
        [sys.executable, os.path.join(_HERE, "zdbk_notifications_data.py"), *args],
...
def test_cli_missing_config():
    ...
    try:
        r = _run(["--type", "zdbk_notifications", "--project-root", _ROOT])
...
def test_real_pull():
    ...
    obj = json.loads(r.stdout)
    assert "notifications" in obj, f"响应缺少 notifications: {r.stdout[:300]}"
    assert isinstance(obj["notifications"], list), "notifications 应为列表"
    assert "total" in obj, f"响应缺少 total: {r.stdout[:300]}"
    if obj["notifications"]:
        n0 = obj["notifications"][0]
        for key in ("id", "title"):
            assert key in n0, f"notification 缺少字段 {key}: {n0}"
...
if __name__ == "__main__":
    ...
    print("全部 zdbk-notifications 测试通过 ✅")
