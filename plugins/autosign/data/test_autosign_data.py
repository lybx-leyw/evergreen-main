...
模式 B：有 .env 凭据时真实轮询 courses 雷达签到（无进行中签到也属正常成功）。
"""
import json
import os
import subprocess
...
def _run(args):
    return subprocess.run([sys.executable, os.path.join(_HERE, "autosign_data.py"), *args],
...
def test_cli_missing_config():
    ...
    try:
        r = _run(["--type", "autosign", "--project-root", _ROOT])
        assert r.returncode == 1
        assert "error" in json.loads(r.stdout)
    finally:
...
def test_real_pull():
    ...
    assert "logs" in obj and isinstance(obj["logs"], list)
    assert "signed" in obj and "total" in obj
