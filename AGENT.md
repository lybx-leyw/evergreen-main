...
| 2026-08-25 | 数据源 manifest 契约统一 + 可选声明：模型 A/B 收敛单一 typed model（`script`/`process` 互斥、`category` 默认「未分类」、TTL 统一 s/m/h/ms、`androidSupport` 严格 bool fail-closed、`id`/`name` 可选）；新增可选 `auth`（引用 config key）/`stream`（hls/mp4/http-flv/sse/stdio-jsonl）/`file`/`process` 增强（scope/autoRestart/protocol），**缺省零行为变化** | core-data（牵头）/ plugins（生成器/示例）/ renderer（消费） | ✅ 已实现（T1/T6），契约见 `core/data/docs/plugin-data-source.md` |
| 2026-08-25 | 平台级凭据/会话中心：`CredentialStore`（统一 SP/config.json/env.json + isSecure + 直写去 `.config_port`）+ `SessionProvider`/`SessionCoordinator`（登录锁：同 provider 并发登录 Future 共享、会话失效单点重登）；`DataType.sessionProviderId`；orchestrator 会话失效→重登→重拉降级；ZJU 12 类型接入 `sessionProviderId:'zju'` | core-config + core-data（牵头）/ renderer（zju 接入，T9） | ✅ 已实现（T2/T9），登录不挤占验收目标 |
| 2026-08-25 | data 层流式能力：`registerStream`/`streamOf`/`streamByName` + `dataChangeEvents` 广播流 + SSE 端点 `GET /data/stream/:name`、`GET /data/events`（零新依赖）；`DataHttpServer` 端点新增 | core-data（牵头）/ renderer（html_modle subscribe 事件驱动化，T7b） | ✅ 已实现（T3/T7b） |
| 2026-08-25 | 降级链第三级 + 进程守护：`PluginRunner.runOnce` 增 `timeout`（超时 kill 子进程，**core/agent/data 三副本同步**）；CLI 数据源 fetcher 60s 超时；空 vs 不可达区分；`DataType.fallback` 静态兜底；`Cache` 写锁；`DataSourceLoader` 崩溃退避重启（1s/3s/9s×3）+ 手动 `restart()` | core（基础设施）+ core-data（牵头） | ✅ 已实现（T4） |
| 2026-08-25 | 平台 Python 库：`scripts/evg_lib/{config,cas,jsonio}.py`（可选 import + `try/except ImportError` 内联 fallback，存量零影响）；`SubprocessRunner`/`ChaquopyRunner` 注入 `PYTHONPATH`（`bindGreenixScriptsDir`）；`PythonSession` 常驻 stdio JSON Lines 封装（阶梯终止） | platform（牵头）/ plugins（生成器 `useEvgLib` 开关，T6） | ✅ 已实现（T5/T6），零新依赖（requests stream 替代） |
| 2026-08-25 | 文件型数据源：`DataFileService.downloadFile/downloadFiles`（headers/超时/退避重试/PathSandbox 防越界，返回 `Result<String>`）+ `FileEntry`/`extractFileEntries` + `registerFile`/`fileOf`/`fileByName`；renderer 侧 `pickExportDirectory`/`FileExportBar` 导出到用户自选路径 | core-services + core-data（牵头）/ renderer（导出 UI，T8b） | ✅ 已实现（T8a/T8b） |
| 2026-08-25 | `save_credential` 去 `.config_port` 硬依赖：scraper/html-creator 两处 `SaveCredentialTool` 统一直写（`CredentialStore.writeCredentialDirect`） | renderer（牵头） | ✅ 已实现（T9） |
