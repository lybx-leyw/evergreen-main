...
var English = Messages{
	...
	SetKeyHint:      "Set your API key, then run `reasonix_gr`.",
	ConfigLabel:     "config",
	ModelsLabel:     "models",
	...
	StepScaffold:    "scaffold reasonix_gr.toml",
	StepSetKey:      "set API key",

	InitHint:       "Project memory (AGENTS.md) is generated in-session: run `reasonix_gr`, then `/init` — the model analyzes the codebase and writes it. For configuration, use `reasonix_gr setup`.",
	StepSetKeyHint: "run `reasonix_gr setup`, or export DEEPSEEK_API_KEY=…",
	StepChatDesc:   "interactive session",
	StepRunDesc:    "one-shot task",
	HelpFooter:     "reasonix_gr help · all commands",

	ChatTip:           "Context is kept across turns. Type 'exit' or Ctrl-D to quit.",
	TurnCancelled:     "cancelled — back to prompt",
	NoSessionToResume: "no saved session to resume — start a new one with `reasonix_gr`",
	...
	OutputStyleHint:    "set agent.output_style in reasonix_gr.toml to apply one (takes effect next session)",
	ThemeHeader:        "themes:",
	...
	SlashMCPNone:                 "no MCP servers configured — add a [[plugins]] entry in reasonix_gr.toml",
	...
	ListHooksNone:       "hooks: none active — configure in .reasonix_gr/settings.json (project, after trust) or ~/.reasonix_gr/settings.json (global)",
	ListMcpHeader:       "mcp servers",
	ListMcpNone:         "mcp: no servers connected — add one in reasonix_gr.toml ([[plugins]]) or a project .mcp.json",
	...
	MissingKeyIntro:       "reasonix_gr.toml is ready — just an API key away.",
	WroteFileFmt:          "Wrote %s",
	SetupComplete:         "Setup complete.",
	SetupCancelled:        "setup cancelled.",
	TryHintFmt:            "Try: %s",
	NextHint:              "Next: set your API key (run `reasonix_gr setup` or export DEEPSEEK_API_KEY=...), then run `reasonix_gr run \"your task\"`.",
	...
	SkipStaleCustomEntryFmt:    "skipping stale %q entry from reasonix_gr.toml (pointing at %s) — please remove it from [[providers]]",
	...
	UsageRunHint:              "usage: reasonix_gr run [--model NAME] <task>",
	ErrorPrefix:               "error:",
	...
	ProviderErrAuth:                "Authentication failed (HTTP 401): your API key is missing or unset. Add it to .env or run `reasonix_gr setup`.",
	ProviderErrAuthRejected:        "Authentication failed (HTTP 401): the server rejected your API key. It may be wrong or expired, or the provider hit a transient auth/quota issue — retried with backoff and still failed. Try again shortly, or check the key in .env / run `reasonix_gr setup`.",
	...
	UsageBody: `reasonix_gr — a config- and plugin-driven coding agent (multi-model)

Usage:
  reasonix_gr [--model NAME] [-c|--continue] [--resume] [--yolo] [--dir PATH]   interactive session (multi-turn; -c resumes the latest, --resume picks one)
  reasonix_gr run  [--model NAME] [--max-steps N] [-c|--continue] [--resume PATH] <task>   run one task and exit
  reasonix_gr review [--base BRANCH] [--commit SHA] [--model NAME]  AI-powered code review on local diffs
  reasonix_gr serve [--model NAME] [--addr HOST:PORT] [--auth none|token|password] [--token STR] [--password STR] [--hash-password]  serve over HTTP+SSE (with optional auth)
  reasonix_gr acp [--model NAME]                           serve Agent Client Protocol over stdio (also: reasonix_gr --acp)
  reasonix_gr setup [path]                                 interactive config wizard; writes reasonix_gr.toml (+ .env)
  reasonix_gr config auto-plan [off|on]                    configure automatic plan mode
  reasonix_gr config memory-v5 [off|on|status]             configure Memory v5
  reasonix_gr config reasoning-language [auto|zh|en]        configure visible reasoning language
  reasonix_gr mcp <add|remove|list|import>                 manage MCP servers in reasonix_gr.toml
  reasonix_gr init                                         show how to generate project memory (AGENTS.md)
  reasonix_gr doctor [--json]                              print redacted local diagnostics
  reasonix_gr bot start|doctor|weixin-login                multi-channel IM bot gateway
  reasonix_gr upgrade [--check] [--force]                   self-update to the latest release (also: reasonix_gr update)
  reasonix_gr version
  reasonix_gr help

Examples:
  reasonix
  reasonix_gr --continue
  reasonix_gr run "implement the TODOs in main.go"
  reasonix_gr run --model mimo-pro "add unit tests for this function"
  echo "explain this code" | reasonix_gr run

Configuration:
  Resolution: flag > ./reasonix_gr.toml > ~/.reasonix_gr/config.toml > built-in defaults
  Secrets come from the environment via api_key_env (e.g. DEEPSEEK_API_KEY).
  Run 'reasonix_gr setup' to scaffold a config; see docs/SPEC.md.
`,
}
