...
import (
	"reasonix_gr/internal/evidence"
	"reasonix_gr/internal/nilutil"
...
const (
	...
	// --- Federation events (Evergreen multi-agent orchestration) ---

	// FederationStarted marks the beginning of a federation run. Federation
	// payload carries the root task and module list.
	FederationStarted
	// FederationCompleted marks a successful federation run.
	FederationCompleted
	// FederationAborted marks a federation run that was aborted or failed.
	FederationAborted

	// TaskCreated fires when the Planner generates a new task node.
	TaskCreated
	// TaskAssigned fires when a task is assigned to an executor.
	TaskAssigned
	// TaskCompleted fires when a task finishes successfully.
	TaskCompleted
	// TaskFailed fires when a task fails.
	TaskFailed

	// ContractProposed fires when a new interface contract is proposed.
	ContractProposed
	// ContractAccepted fires when both sides approve a contract.
	ContractAccepted
	// ContractRejected fires when a contract is rejected.
	ContractRejected
	// ContractViolated fires when code breaks an active contract.
	ContractViolated

	// ExperienceProposed fires when a new experience card is drafted.
	ExperienceProposed
	// ExperienceApproved fires when the Librarian approves a card.
	ExperienceApproved
	// ExperienceSuperseded fires when a card is superseded by a newer version.
	ExperienceSuperseded

	// CreditChanged fires when an agent's credit score is adjusted.
	CreditChanged
	// ReviewSubmitted fires when a Keeper submits a code review.
	ReviewSubmitted
	// ReviewApproved fires when a review is approved.
	ReviewApproved
	// ReviewRejected fires when a review is rejected (changes requested).
	ReviewRejected

	// KeeperAssigned fires when a Module Keeper is assigned to a module.
	KeeperAssigned
	// ModuleHealthChanged fires when a module's health status changes.
	ModuleHealthChanged

	// DebateStarted fires when a debate round begins between agents.
	DebateStarted
	// DebateEnded fires when a debate round concludes.
	DebateEnded
	// ErrorBudgetExhausted fires when a module's error budget is consumed.
...
// FederationPayload carries federation-specific data for Evergreen multi-agent
// orchestration events. Fields are populated per Kind as documented.
type FederationPayload struct {
	// Task carries task-level data (TaskCreated, TaskAssigned, TaskCompleted, TaskFailed).
	Task *FederationTask `json:"task,omitempty"`
	// Contract carries contract-level data (ContractProposed, ContractAccepted,
	// ContractRejected, ContractViolated).
	Contract *FederationContract `json:"contract,omitempty"`
	// Experience carries experience-card data (ExperienceProposed, ExperienceApproved,
	// ExperienceSuperseded).
	Experience *FederationExperience `json:"experience,omitempty"`
	// Credit carries credit-score change data (CreditChanged).
	Credit *FederationCredit `json:"credit,omitempty"`
	// Review carries review data (ReviewSubmitted, ReviewApproved, ReviewRejected).
	Review *FederationReview `json:"review,omitempty"`
	// System carries system-level data (FederationStarted, FederationCompleted,
	// FederationAborted, KeeperAssigned, ModuleHealthChanged, ErrorBudgetExhausted).
	System *FederationSystem `json:"system,omitempty"`
}

// FederationTask describes a task-level federation event.
...
// FederationContract describes a contract-level federation event.
...
// FederationExperience describes an experience-card federation event.
type FederationExperience struct {
...
// FederationCredit describes a credit-score change event.
type FederationCredit struct {
...
// FederationReview describes a review-level federation event.
type FederationReview struct {
...
// FederationSystem describes a system-level federation event.
...
type Event struct {
	...
	// Federation carries Evergreen multi-agent orchestration data
	// (FederationStarted..ErrorBudgetExhausted).
