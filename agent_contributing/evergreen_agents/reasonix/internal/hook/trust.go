...
// Trust gates project hooks. A project's .reasonix_gr/settings.json can run
...
// root. The trust flag lives in user-global state (~/.reasonix_gr/trust.json),
// NOT in the project file itself — an attacker controls the latter. Global
// hooks (~/.reasonix_gr/settings.json) are the user's own and always run.
