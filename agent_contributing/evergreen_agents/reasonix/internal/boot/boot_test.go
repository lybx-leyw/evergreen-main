...
import (
	...
	"reasonix_gr/internal/agent/testutil"
	"reasonix_gr/internal/config"
	"reasonix_gr/internal/event"
	"reasonix_gr/internal/memory"
	"reasonix_gr/internal/netclient"
	"reasonix_gr/internal/plugin"
	"reasonix_gr/internal/provider"
	"reasonix_gr/internal/sandbox"
	"reasonix_gr/internal/tool"
	"reasonix_gr/internal/tool/builtin"
...
func TestBuildFoldsProjectMemoryIntoSystemPrompt(t *testing.T) {
	...
	writeFile(t, dir, "reasonix_gr.toml", `
...
api_key_env = "REASONIX_TEST_KEY_UNSET"
`)
	writeFile(t, dir, ".reasonix_gr/skills/projskill.md", "---\ndescription: a project skill\n---\nplaybook")
...
model = "x"
`)
	writeFile(t, dir, ".reasonix_gr/skills/readonlydig/SKILL.md", `---
...
func TestBuildTokenEconomyPlanModeCanConnectAllowedMCPSource(t *testing.T) {
	...
	setBootTokenProfileTestProvider(t, prov)
	writeFile(t, dir, "reasonix_gr.toml", fmt.Sprintf(`
default_model = "test-model"
...
func TestBuildOmitsExcludedSkillRootsFromPromptAndRuntimeList(t *testing.T) {
	...
	writeFile(t, home, ".reasonix_gr/skills/keep.md", "---\ndescription: keep\n---\nplaybook")
...
func TestRememberPermissionRuleUsesWorkspaceRoot(t *testing.T) {
	...
	t.Chdir(cwd)
	writeFile(t, cwd, "reasonix_gr.toml", `
[permissions]
allow = ["Bash(cwd*)"]
`)
	writeFile(t, workspace, "reasonix_gr.toml", `
[permissions]
...
`)
	...
	cwdCfg := config.LoadForEdit(filepath.Join(cwd, "reasonix_gr.toml"))
	...
	workspaceCfg := config.LoadForEdit(filepath.Join(workspace, "reasonix_gr.toml"))
...
`)
	...
	if !res.Saved || res.Path != filepath.Join(workspace, "reasonix_gr.toml") {
...
`)
	...
	if _, err := os.Stat(filepath.Join(cwd, "reasonix_gr.toml")); !os.IsNotExist(err) {
...
`)
	...
	cfg := config.LoadForEdit(filepath.Join(workspace, "reasonix_gr.toml"))
...
// ~/.reasonix_gr/config.json with no v1+ config present must be imported during
...
func TestBuildMigratesLegacyConfigEndToEnd(t *testing.T) {
	...
	// migrated plugins.
	writeFile(t, proj, "reasonix_gr.toml", "")
	writeFile(t, filepath.Join(home, ".reasonix_gr"), "config.json",
		`{"apiKey":"sk-e2e","lang":"zh","mcpServers":{"fs":{"command":"npx","args":["-y","server-fs"]}}}`)
	writeFile(t, filepath.Join(home, ".reasonix_gr", "sessions"), "chat-1.events.jsonl",
...
func TestBuildMigratesLegacyXDGAndProjectSessions(t *testing.T) {
	...
	proj := robustTempDir(t)
	writeFile(t, proj, "reasonix_gr.toml", "[codegraph]\nenabled = false\n")

	legacyRoot := filepath.Join(xdg, "reasonix_gr")
...
`)
	...
	raw, err := os.ReadFile(filepath.Join(dir, "reasonix_gr.toml"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestBuildDefaultsToNearestGitRoot(t *testing.T) {
	...
	writeFile(t, root, "reasonix_gr.toml", `
