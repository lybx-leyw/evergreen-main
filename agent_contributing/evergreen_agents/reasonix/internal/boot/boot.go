...
import (
	...
	"reasonix_gr/internal/command"
	"reasonix_gr/internal/config"
	"reasonix_gr/internal/control"
	"reasonix_gr/internal/event"
	"reasonix_gr/internal/guardian"
	"reasonix_gr/internal/history"
	"reasonix_gr/internal/hook"
	"reasonix_gr/internal/installsource"
	"reasonix_gr/internal/instruction"
	"reasonix_gr/internal/jobs"
	"reasonix_gr/internal/lsp"
	"reasonix_gr/internal/memory"
	"reasonix_gr/internal/memorycompiler"
	"reasonix_gr/internal/migration"
	"reasonix_gr/internal/netclient"
	"reasonix_gr/internal/outputstyle"
	"reasonix_gr/internal/permission"
	"reasonix_gr/internal/planmode"
	...
	"reasonix_gr/internal/tool/sessiontool"
)
...
type Options struct {
	...
	// controller but are not persisted to reasonix_gr.toml.
...
func Build(ctx context.Context, opts Options) (*control.Controller, error) {
	...
	// Custom slash commands (.reasonix_gr/commands + user dir). Best-effort: a malformed
...
func rememberPermissionConfigPath(workspaceRoot string) string {
	...
	if workspaceRoot != "" {
		return filepath.Join(workspaceRoot, "reasonix_gr.toml")
	}
	path := config.SourcePath()
	if path == "" {
		path = "reasonix_gr.toml" // match Config.Save() fallback
	}
	return path
}
