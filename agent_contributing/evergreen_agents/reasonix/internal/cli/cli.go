...
import (
	...
	"reasonix_gr/internal/control"
	"reasonix_gr/internal/event"
	"reasonix_gr/internal/i18n"
	"reasonix_gr/internal/notify"
	"reasonix_gr/internal/provider"
	"reasonix_gr/internal/provider/openai"
	"reasonix_gr/internal/serve"
	"time"
...
func Run(args []string, version string) int {
	...
	case "init":
		...
		// for config), so `reasonix_gr init` isn't a dead end.
		configureCLIThemeFromConfigNoProbe()
		return initHint()
	case "acp":
		configureCLIThemeFromConfigNoProbe()
	...
	case "upgrade", "update":
		...
		// --- Evergreen federation commands ---
		case "bootstrap":
			return bootstrapCommand(rest)
		case "experience":
			return experienceCommand(rest)
		case "module":
			return moduleCommand(rest)
		case "inspect":
			return inspectCommand(rest)
		case "federation":
			return federationCommand(rest)
		case "agent":
			return agentCommand(rest)
		case "ceo":
			return ceoCommand(rest)
		case "audit":
			return auditCommand(rest)
	case "version", "--version", "-v":
		fmt.Println("reasonix_gr", version)
		return 0
	case "help", "--help", "-h":
		usage()
		return 0
...
func runServe(args []string) int {
	...
	*model = modelForResumePath(*model, *resume, cfg)
	...
	fmt.Printf("reasonix_gr serve — %s on http://%s\n", ctrl.Label(), *addr)
...
func chatREPL(args []string) int {
	fs := flag.NewFlagSet("reasonix_gr", flag.ContinueOnError)
	model := fs.String("model", "", "provider name (default: config default_model)")
	maxSteps := fs.Int("max-steps", 0, "max tool-call rounds (0 = use config/default)")
	cont := fs.Bool("continue", false, "resume the most recent saved session")
	fs.BoolVar(cont, "c", false, "shorthand for --continue")
...
// project-local reasonix_gr.toml only when the user config dir can't be resolved.
func defaultConfigTarget() string {
	if p := config.UserConfigPath(); p != "" {
		return p
	}
	return "reasonix_gr.toml"
}
...
// resolveSetupTargets picks where `reasonix_gr setup` writes. Keys always go to the
// global env. The config goes to the user-global dir by default, to ./reasonix_gr.toml
...
// setupConfig runs the configuration wizard (the `reasonix_gr setup` command),
// writing config.toml to the user-global dir (or ./reasonix_gr.toml under --local)
...
// initHint handles `reasonix_gr init`. Unlike a config scaffold, project memory is
...
// (and to `reasonix_gr setup` for config) so the verb isn't a dead end.
func initHint() int {
...
func interactiveSetup(configPath, envPath string) int {
	...
	fmt.Print(boxed([]string{
		accent("◆") + " " + fmt.Sprintf(i18n.M.WelcomeTitleFmt, bold("reasonix_gr")),
		"",
...
// that older versions of the wizard wrote into reasonix_gr.toml. They collide
...
// can warn the user to clean up reasonix_gr.toml by hand.
...
// duplicate provider entries on subsequent re-runs of `reasonix_gr setup`.
// The host-based slug also gives users a meaningful name to grep for in
// reasonix_gr.toml. Falls back to a short sha1 of the raw URL when the URL
...
// not exist in config because users editing reasonix_gr.toml deal with SKU names
// directly.
...
// are appended — so re-running `reasonix_gr setup` with a corrected key replaces the
...
func configReasoningLanguageCommand(args []string) int {
	...
	local := fs.Bool("local", false, "write ./reasonix_gr.toml instead of the user config")
	if err := fs.Parse(args); err != nil {
		return 2
	}
	rest := fs.Args()
...
func configUsage() {
	fmt.Print(`Usage:
  reasonix_gr config auto-plan [off|on]
  reasonix_gr config memory-v5 [off|on|status]
  reasonix_gr config reasoning-language [--local] [auto|zh|en]
`)
}
