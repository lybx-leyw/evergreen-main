...
func TestMCPMigrationWaitsForCLIWorkspace(t *testing.T) {
	isolateCLIConfigHome(t)
	cwd := mustGetwd(t)
	if err := os.WriteFile(filepath.Join(cwd, "reasonix_gr.toml"), []byte(`
[[plugins]]
name = "cwd-project"
...
func TestRunMigratesLegacyConfigBeforeConfigOnlyCommands(t *testing.T) {
	isolateCLIConfigHome(t)
	legacyPath := filepath.Join(filepath.Dir(config.UserConfigPath()), "reasonix_gr.toml")
...
func TestConfigAutoPlanLocalIsRejected(t *testing.T) {
	...
	if _, err := os.Stat("reasonix_gr.toml"); !os.IsNotExist(err) {
		t.Fatalf("reasonix_gr.toml should not be written, stat err=%v", err)
	}
...
func TestConfigAutoPlanIgnoresProjectConfig(t *testing.T) {
	...
	if err := os.WriteFile("reasonix_gr.toml", []byte("[agent]\nauto_plan = \"on\"\n"), 0o644); err != nil {
...
func TestConfigReasoningLanguageLocalCreatesMinimalProjectOverride(t *testing.T) {
	...
	body, err := os.ReadFile("reasonix_gr.toml")
	if err != nil {
...
// into reasonix_gr.toml. These collide with the wizard's own menu items, so
...
func TestWithBuiltinFamiliesDoesNotAddMissingMimo(t *testing.T) {
	// The user's case: a reasonix_gr.toml that defines only deepseek providers.
...
func TestWriteDefaultConfigOmitsLegacyInternalMCPSections(t *testing.T) {
	path := filepath.Join(t.TempDir(), "reasonix_gr.toml")
