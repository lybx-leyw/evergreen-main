...
type chatTUI struct {
	...
	// commands are custom slash commands loaded from .reasonix_gr/commands; each renders
...
var cliImageRefRe = regexp.MustCompile(`(?:^|\s)@\.reasonix_gr/attachments/clipboard-\d{8}-\d{6}\.\d+(?:-(?:\d{6}|[a-f0-9]{8}))?\.(?:png|jpg|jpeg|gif|webp)`)
