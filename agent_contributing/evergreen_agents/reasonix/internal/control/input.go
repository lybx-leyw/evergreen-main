...
- Say briefly in the first visible reply that the goal is being handled with AutoResearch and that state will live under .reasonix_gr/autoresearch/<task-id>/.
- Keep dynamic state out of REASONIX.md, AGENTS.md, project memory, system prompts, and tool schemas. Use project-local .reasonix_gr/autoresearch/ state only.
- For a new task, create a collision-resistant task id YYYYMMDD-HHMMSS-slug, check .reasonix_gr/autoresearch/ first, and append -2, -3, etc. only on collision. Reuse an explicitly supplied .reasonix_gr/autoresearch/<task-id>/ path exactly.
...
func shouldAutoStartResearchGoal(input string) bool {
	...
	if strings.Contains(lower, ".reasonix_gr/autoresearch/") {
		return true
	}
