...
func TestRemoveMCPServerRemovesUnconnectedLazyPlaceholder(t *testing.T) {
	...
	t.Chdir(dir)
	if err := os.WriteFile("reasonix_gr.toml", []byte(`
[[plugins]]
name = "mock"
command = "mock-mcp"
tier = "lazy"
...
func TestApprovalPersistentBashPrefixRememberRule(t *testing.T) {
	...
	c := New(Options{
		...
		OnRemember: func(rule string) RememberResult {
			remembered = rule
			return RememberResult{Rule: rule, Path: "reasonix_gr.toml", Saved: true}
		},
	})
	go func() {
	...
	if len(notices) != 1 || !strings.Contains(notices[0], "Bash(go test:*)") || !strings.Contains(notices[0], "reasonix_gr.toml") {
...
func TestReloadCommandsFromFilesystem(t *testing.T) {
	...
	wsRoot := t.TempDir()
	cmdDir := filepath.Join(wsRoot, ".reasonix_gr", "commands")
...
// (higher priority) wins. ConventionDirs = [".reasonix_gr", ".agents", ".agent",
...
func TestReloadCommandsSameNameAcrossDirs(t *testing.T) {
	...
	// Higher priority: .reasonix_gr/commands
	reasonixDir := filepath.Join(wsRoot, ".reasonix_gr", "commands")
