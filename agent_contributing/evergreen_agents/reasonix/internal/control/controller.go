...
import (
	...
	"reasonix_gr/internal/billing"
	"reasonix_gr/internal/checkpoint"
	"reasonix_gr/internal/command"
	"reasonix_gr/internal/config"
	"reasonix_gr/internal/diff"
...
type Controller struct {
	...
	// reasonix_gr.toml on add/remove, building specs from entries). See mcp.go.
	mcp mcpManager
...
// out of project reasonix_gr.toml while preserving the CLI AddMCPServer semantics.
...
func listSourceDirs(root string, maxDepth int) []string {
	skip := map[string]bool{
		...
		"vendor": true, ".reasonix_gr": true, "desktop": true,
