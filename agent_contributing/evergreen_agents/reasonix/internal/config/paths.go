...
func reasonixHomeDir() string {
	...
	if runtime.GOOS != "windows" {
		if home, err := os.UserHomeDir(); err == nil && home != "" {
			return filepath.Join(home, ".reasonix_gr")
		}
		return ""
	}
	dir := osUserConfigDir()
	...
	return filepath.Join(dir, "reasonix_gr")
}
...
func legacyXDGConfigPaths() []string {
	...
	if home, err := os.UserHomeDir(); err == nil && home != "" {
		add(filepath.Join(home, ".config", "reasonix_gr", "config.toml"))
	}
	return paths
}
...
func legacyOSSupportDir() string {
	...
	path := filepath.Join(dir, "reasonix_gr")
...
func userConfigDisplayPath() string {
	p := userConfigPath()
	if p == "" {
		return "<os-config-dir>/reasonix_gr/config.toml"
	}
	if home, err := os.UserHomeDir(); err == nil && home != "" {
...
// REASONIX_HOME/config.toml, then ~/.reasonix_gr/config.toml on Unix-like systems,
// or %AppData%/reasonix_gr/config.toml on Windows. "" when the user config dir
// can't be resolved.
...
// .reasonix_gr/settings.json (see internal/hook).
var ConventionDirs = []string{".reasonix_gr", ".agents", ".agent", ".claude"}
...
// ~/.reasonix_gr/commands), the Reasonix home commands dir, the legacy OS
// app-support dir if different, then the project's
// convention dirs (.claude/commands … .reasonix_gr/commands). Scanning the .claude /
...
func SourcePathForRoot(root string) string {
	root = resolveRoot(root)
	projectTOML := "reasonix_gr.toml"
	if root != "." {
		projectTOML = filepath.Join(root, "reasonix_gr.toml")
	}
