...
// legacyConfig is the subset of the v0.x (~/.reasonix_gr/config.json) schema this
// import carries forward. Fields absent here are dropped on purpose: desktop tab
// state is frontend-owned, and skills already live in the shared ~/.reasonix_gr/skills
...
// checks v1-era TOML first, then v0.5/v0.x ~/.reasonix_gr/config.json, and never
...
func MigrateLegacyIfNeededForRoot(root string) (*MigrationResult, error) {
	...
	src := filepath.Join(home, ".reasonix_gr", "config.json")
...
func migrateMCPToUserConfig(projectRoots []string) (*MCPGlobalMigrationResult, error) {
	...
	for _, root := range normalizedMCPMigrationRoots(projectRoots) {
		addEntries(loadPluginEntriesFromTOML(filepath.Join(root, "reasonix_gr.toml")))
...
func legacyTOMLPaths(dest, home string) []string {
	...
	for _, legacy := range legacyXDGConfigPaths() {
		add(legacy)
		add(filepath.Join(filepath.Dir(legacy), "reasonix_gr.toml"))
	}
	add(filepath.Join(filepath.Dir(dest), "reasonix_gr.toml"))
	if home != "" {
		add(filepath.Join(home, ".reasonix_gr", "reasonix_gr.toml"))
	}
	return paths
}
