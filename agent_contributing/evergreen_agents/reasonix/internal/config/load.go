...
// the v0.x ~/.reasonix_gr/config.json's mcpServers. Provider api_key_env values
...
// each project's reasonix_gr.toml + .mcp.json are resolved independently without
...
func LoadForRoot(root string) (*Config, error) {
	...
	// project config's other fields, but do not let ./reasonix_gr.toml override
	...
	// project reasonix_gr.toml doesn't drop the global config's MCP servers.
	...
	// reasonix_gr.toml wins on a name collision (see mergeMCPJSON).
	mcpFile := mcpJSONFile
	if root != "." {
	...
	// Lowest priority: the v0.x ~/.reasonix_gr/config.json's mcpServers, so upgrading
...
// MiMo providers needed by legacy refs that live outside reasonix_gr.toml, such as
