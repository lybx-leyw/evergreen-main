...
import (
	...
	"reasonix_gr/internal/fileutil"
	"reasonix_gr/internal/mcpdiag"
...
// .mcp.json-only server would otherwise be serialized into reasonix_gr.toml or the
...
func pluginTOMLSourcePath(name string) string {
	paths := append([]string{"reasonix_gr.toml"}, userConfigCandidatePaths()...)
...
// half-written reasonix_gr.toml that fails to parse on next load. Parent directories
// are created as needed.
//
// For project configs (./reasonix_gr.toml) the write is incremental: only sections
...
// (SourcePath), or to ./reasonix_gr.toml when none exists yet — the conventional
...
// user's global config when root has no reasonix_gr.toml. Existing project files
