...
func expectedDefaultReasonixHome(home string) string {
	if runtime.GOOS == "windows" {
		return filepath.Join(home, "AppData", "Roaming", "reasonix_gr")
	}
...
func TestUserConfigDisplayPathCollapsesHome(t *testing.T) {
	...
	if !strings.HasSuffix(got, "reasonix_gr/config.toml") {
		t.Fatalf("display path = %q, want reasonix_gr/config.toml suffix", got)
	}
...
func TestRenderTOMLDefaultStepsDoNotOverrideGlobalConfig(t *testing.T) {
	...
	projectPath := filepath.Join(projectDir, "reasonix_gr.toml")
