...
func legacyHome(t *testing.T) (src, dest, home string) {
	...
	return filepath.Join(home, ".reasonix_gr", "config.json"), userConfigPath(), home
}
...
func TestMigrateMCPToUserConfigOnUpgradeCollectsKnownSources(t *testing.T) {
	...
	writeLegacy(t, filepath.Join(filepath.Dir(dest), "reasonix_gr.toml"), `
[[plugins]]
name = "legacy-toml"
...
`), 0o644); err != nil {
	...
	if err := os.WriteFile(filepath.Join(projectTOML, "reasonix_gr.toml"), []byte(`
[[plugins]]
name = "project-toml"
...
`), 0o644); err != nil {
	...
	if err := os.WriteFile(filepath.Join(lateProject, "reasonix_gr.toml"), []byte(`
[[plugins]]
name = "late"
command = "late-bin"
`), 0o644); err != nil {
...
func TestMigrateMCPToUserConfigOnUpgradePreservesConfigVersion(t *testing.T) {
	...
	project := t.TempDir()
	if err := os.WriteFile(filepath.Join(project, "reasonix_gr.toml"), []byte(`
[[plugins]]
name = "project"
command = "project-bin"
`), 0o644); err != nil {
...
func TestMigrateImportsLegacyV1TOMLBeforeJSON(t *testing.T) {
	srcJSON, dest, _ := legacyHome(t)
	legacyTOML := filepath.Join(filepath.Dir(dest), "reasonix_gr.toml")
...
func TestMigrateImportsLegacyV1HomeTOMLBeforeJSON(t *testing.T) {
	...
	legacyTOML := filepath.Join(home, ".reasonix_gr", "reasonix_gr.toml")
	if err := os.MkdirAll(filepath.Dir(legacyTOML), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(legacyTOML, []byte(`
...
func TestMigrateImportsLegacyXDGConfigToPrimaryConfig(t *testing.T) {
	...
	legacy := filepath.Join(home, ".config", "reasonix_gr", "config.toml")
