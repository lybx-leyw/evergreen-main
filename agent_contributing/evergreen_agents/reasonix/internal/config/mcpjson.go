...
// legacyConfigPath is the v0.x (TypeScript line) config file, ~/.reasonix_gr/config.json.
func legacyConfigPath() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return ""
	}
	return filepath.Join(home, ".reasonix_gr", "config.json")
}

// loadLegacyMCP reads the v0.x ~/.reasonix_gr/config.json and returns its enabled
...
// already declare. reasonix_gr.toml's [[plugins]] win on a name collision: it is the
