...
func TestSaveToRoundTrips(t *testing.T) {
	...
	path := filepath.Join(t.TempDir(), "nested", "reasonix_gr.toml")
...
func TestSaveToScopesUserAndProjectFiles(t *testing.T) {
	...
	projectPath := filepath.Join(t.TempDir(), "reasonix_gr.toml")
...
`), 0o644); err != nil {
	...
	if err := os.WriteFile(filepath.Join(root, "reasonix_gr.toml"), []byte(`
[[providers]]
name = "deepseek-flash"
kind = "openai"
base_url = "https://api.deepseek.com"
...
`), 0o644); err != nil {
	...
	projectPath := filepath.Join(root, "reasonix_gr.toml")
