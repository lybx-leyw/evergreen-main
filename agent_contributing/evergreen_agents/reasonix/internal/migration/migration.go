...
func migrateLegacyMemorySources(sink event.Sink, verbose bool) memoryMigrationResult {
	...
	if home, herr := os.UserHomeDir(); herr == nil {
		addRoot(filepath.Join(home, ".reasonix_gr"), "~/.reasonix_gr")
	}
...
func migrateLegacySessionSources(sink event.Sink, verbose bool) sessionMigrationResult {
	...
	if home, herr := os.UserHomeDir(); herr == nil {
		reasonixHome := filepath.Join(home, ".reasonix_gr")
		addFlatSource(filepath.Join(reasonixHome, "sessions"), "~/.reasonix_gr/sessions", agent.MigrateLegacySessions)
