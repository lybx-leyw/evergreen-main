...
class _CourseOfferingsScreenState extends ConsumerState<CourseOfferingsScreen> {
  ...
  List<CourseOffering> _filter(List<CourseOffering> all) {
  ...
  Widget build(BuildContext context) {
    final offeringsAsync = ref.watch(courseOfferingsProvider((year: _year, semester: _semester)));

    final allOfferings = offeringsAsync.whenOrNull<List<CourseOffering>>(
          data: (result) => result.fold((o) => o, (_) => []),
        ) ??
        [];

    final filtered = _filter(allOfferings);
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          PopupMenuButton<int>(tooltip: '选择学年', icon: const Icon(Icons.calendar_month),
            onSelected: (y) => setState(() { _year = y; _searchQuery = ''; _searchController.clear(); }),
            itemBuilder: (_) => [
          ...
          PopupMenuButton<int>(tooltip: '选择学期', icon: const Icon(Icons.school),
            onSelected: (s) => setState(() { _semester = s; _searchQuery = ''; _searchController.clear(); }),
            itemBuilder: (_) => [
      ...
      body: offeringsAsync.when(
        ...
        error: (_, __) => Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          ElevatedButton.icon(icon: const Icon(Icons.refresh), label: const Text('重试'), onPressed: _refresh),
        ])),
        data: (result) => result.fold(
          (_) => allOfferings.isEmpty && !_loading
              ...
              : Column(children: [
                  ...
                  Padding(
                    ...
                    child: Align(alignment: Alignment.centerLeft,
                      child: Text(_searchQuery.isNotEmpty ? '找到 ${filtered.length} 门（共 ${allOfferings.length} 门）' : '共 ${allOfferings.length} 门课程',
